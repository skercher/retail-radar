    ...
    <main className="h-[100dvh] flex flex-col bg-zinc-950 text-white overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside
          ...
          `}
        >
          {/* Mobile Stats / Saved Header */}
          <div className="md:hidden flex items-center justify-between px-4 py-2 bg-zinc-900/50 border-b border-zinc-800 text-xs">
            {viewMode === 'saved' ? (
              <>
                <span className="text-zinc-400 font-medium">Saved Properties</span>
                <span className="text-blue-400">{savedProperties.size} saved</span>
              </>
            ) : (
              <>
                <span className="text-zinc-400">{stats.count} properties</span>
          ...
          <div className="flex-1 overflow-y-auto p-4 pb-20 md:pb-4 space-y-3">
            {loading ? (
            ...
            ) : viewMode === 'saved' && savedProperties.size === 0 ? (
              <div className="text-center py-12">
                <Building2 className="w-12 h-12 text-zinc-700 mx-auto mb-3" />
                <p className="text-zinc-500">No saved properties</p>
                <p className="text-zinc-600 text-sm mt-1">
                  Tap the heart icon to save properties
                </p>
              </div>
            ) : (viewMode === 'saved' ? filteredProperties.filter(p => savedProperties.has(p.id)) : filteredProperties).length === 0 ? (
              <div className="text-center py-12">
                <Building2 className="w-12 h-12 text-zinc-700 mx-auto mb-3" />
              ...
              (viewMode === 'saved' ? filteredProperties.filter(p => savedProperties.has(p.id)) : filteredProperties).map((property) => (
                <PropertyCard
                  key={property.id}
                  property={property}
        ...
        <div
          className={`
            ${viewMode === 'list' || viewMode === 'saved' ? 'hidden' : 'flex'}
            md:flex
            flex-1 relative
          `}
        >
